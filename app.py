...
def main():
    st.title("Diabetes Risk Prediction for Women")
    st.sidebar.title("Diabetes Risk Prediction for Women")
    st.markdown("ML Web App for Medical Diagnosis")
    st.sidebar.markdown("ML Web App for Medical Diagnosis")
    ...
    if st.sidebar.checkbox("Custom Input",False):
        ...
        preg = st.number_input('No. of Pregnancies')
        gluc = st.number_input('Glucose Level')
        bp = st.number_input('Blood Pressure')
        skf = st.number_input('Skinfold')
        ins = st.number_input('Insulin')
        bmi = st.number_input('BMI')
        ped = st.number_input('Pedigree')
        age = st.number_input('Age')

        classify = st.sidebar.selectbox("Load Model", ("Logistic Regression","Support Vector Machine (SVM)","K-Nearest Neighbor","Decision Tree", "Random Forest", "XGBoost"))
        ...
        if st.button("Predict"): 
            classifier = classify_select()
            result = classifier.predict([[preg, gluc, bp, skf, ins, bmi, ped, age]])
            # st.write(result)
            if result ==0:
                pred = 'No Risk of Diabetes'
            else:
                pred = 'Risk of Diabetes'
            
            st.success('You are at {}'.format(pred))
    ...
    if st.sidebar.checkbox("Show raw data", False):
        st.subheader("Medical Data Set (Prediction)")
        st.write(df)
